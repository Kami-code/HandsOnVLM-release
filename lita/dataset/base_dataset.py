...
from lita.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
def _mask_targets(target, tokenized_lens, speakers):
    """
    the target is initialized as input_ids,
    and we set the head and human said words to IGNORE_INDEX
    this is because we only want to use the system's words to supervise the training
    """
    # cur_idx = 0
...
def preprocess_multimodal(sources: Sequence[str], data_args: DataArguments) -> Dict:
    """
    sources: a list of conversations, each conversation is a list of multi-term sentences.
    ...
    for source in sources:
        for sentence in source:
            """
            sentence is a dict with keys: 'from' and 'value'
            from means the speaker of the sentence, and value is the content of the sentence
            """
            # in the following if basically we do
            # Input: <image>\n some text...
            # Output: <image>\n some text...
            if DEFAULT_IMAGE_TOKEN in sentence['value']:
                # DEFAULT_IMAGE_TOKEN is <image>
...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if isinstance(i, int):
            sources = [sources]  # change source to a list
