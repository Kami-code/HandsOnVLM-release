...
from lita.constants import IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, TIME_TOKEN_TEMPLATE, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
    ...
    for (input_ids, image_tensor), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        with torch.inference_mode():
            output_ids = model.generate(
                input_ids,
                image=image_tensor.to(dtype=torch.float16, device='cuda', non_blocking=True),
