...
class ModelArguments:
    ...
    fuse_input_mode: str = 'origin'
    backbone: str = 'liha'
    traj_decoder: str = 'MLP'
    n_bins: int = 400
...
class DataArguments:
    ...
    epic_kitchen_sample_rate: list[float]
    hoi_ref_sample_rate: list[float]
    ...
    epic_kitchen_data: str = 'narration_conversation'
    hoi_ref_data: str = 'epic||ego4d'
    samples_per_epoch: int = 15000  # 500 * 8 * 2 * 10
    ek_version: str = 'ek100'
    ek_conversation_rephrase_rate: float = 0.5
    epic_kitchen_use_percentage: float = 0.2
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    lambda_obj: float = 1e-1
    lambda_obj_kl: float = 1e-3
    lambda_traj: float = 1
    lambda_traj_kl: float = 1e-3
    lambda_last_hand: float = 0.001
    hoi_lambda: float = 1
