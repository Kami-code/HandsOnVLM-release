...
from llava.mm_utils import tokenizer_image_token, tokenizer_image_and_traj_token
...
def preprocess_v1(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            conv.append_message(role, sentence["value"])
            """"
            j = 0, sentence: {'from': 'human', 'value': '<image>\nWhere should my hand move to if I want to {}?'}
            role: USER
            sentence['value']: '<image>\nWhere should my hand move to if I want to {}?'
            
            j = 1, sentence: {'from': 'gpt', 'value': 'Sure! Here is the hand trajectory <hand_traj>.</s>'}
            role: ASSISTANT
            sentence['value']: 'Sure! Here is the hand trajectory <hand_traj>.</s>'
            """
        conversations.append(conv.get_prompt())
        """
        conv.get_prompt(): A chat between a curious user and an artificial intelligence assistant. 
    ...
    if has_image:
        # input_ids = torch.stack([tokenizer_image_and_traj_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations], dim=0)
        input_ids = torch.stack([tokenizer_image_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations], dim=0)
    else:
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    ...
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.PLAIN:
        print("Using plain style")
        return preprocess_plain(sources, tokenizer)
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.LLAMA_2:
        print("Using llama2 style")
        return preprocess_llama_2(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.version.startswith("v1"):
        # print("Using v1 style")
        # todo: this is the current default
        return preprocess_v1(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.version == "mpt":
        print("Using mpt style")
